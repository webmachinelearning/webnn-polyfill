import {GruCellOptions, GruOptions, ModelBuilder, RecurrentNetworkActivation, RecurrentNetworkDirection, RecurrentNetworkWeightLayout} from '../model_builder';
  ...
  static buildWithOptions(
    ...
    for (let slot = 0; slot < numDirections; ++slot) {
      cellWeight.push(builder.squeeze(
          builder.slice(weight, [slot, 0, 0], [1, -1, -1]), {axes: [0]}));
      cellRecurrentWeight.push(builder.squeeze(
          builder.slice(recurrentWeight, [slot, 0, 0], [1, -1, -1]),
          {axes: [0]}));
      cellBias.push(
          bias ? (builder.squeeze(
                     builder.slice(bias, [slot, 0], [1, -1]), {axes: [0]})) :
                 null);
      cellRecurrentBias.push(
          recurrentBias ? (builder.squeeze(
                              builder.slice(recurrentBias, [slot, 0], [1, -1]),
                              {axes: [0]})) :
                          null);
    }
    ...
    for (let step = 0; step < steps; ++step) {
      ...
      for (let slot = 0; slot < numDirections; ++slot) {
        cellHidden.push(builder.squeeze(
            builder.slice(hiddenState, [slot, 0, 0], [1, -1, -1]),
            {axes: [0]}));
      }
      ...
      for (let slot = 0; slot < numDirections; ++slot) {
        ...
        const cellInput = builder.squeeze(
            builder.slice(input, [slice, 0, 0], [1, -1, -1]), {axes: [0]});
