...
describe('test mobilenetv2 batchnorm nchw', function() {
  ...
  before(async () => {
    ...
    async function buildLinearBottleneck(
      ...
      const batchNorm2 =
          await buildConvBatchNorm(builder.relu(batchNorm1), '2', subName);
    ...
    const batch0 = await buildConvBatchNorm(data, '0', '', {strides, padding});
    ...
    const bottleneck11 =
        await buildLinearBottleneck(bottleneck10, '11', {padding, groups: 576});
    const bottleneck12 =
        await buildLinearBottleneck(bottleneck11, '12', {padding, groups: 576});
    const bottleneck13 = await buildLinearBottleneck(
        bottleneck12, '13', {strides, padding, groups: 576}, false);
    const bottleneck14 =
        await buildLinearBottleneck(bottleneck13, '14', {padding, groups: 960});
    const bottleneck15 =
        await buildLinearBottleneck(bottleneck14, '15', {padding, groups: 960});
    ...
    const conv0Weight =
        await utils.buildConstantFromNpy(builder, new URL(conv0WeightUrl, url));
