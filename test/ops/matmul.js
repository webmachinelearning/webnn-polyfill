describe('test matmul', function() {
  ...
  it('matmul 1d', async function() {
    ...
    const b = nn.constant(
        {type: 'tensor-float32', dimensions: [4]},
    ...
    execution.setInput(
        'a', new Float32Array([0.9025404, 0.89538723, 0.16789329, 0.7440875]));
  ...
  it('matmul 1dx2d', async function() {
    const a = nn.input('a', {type: 'tensor-float32', dimensions: [4]});
    const b = nn.constant(
        {type: 'tensor-float32', dimensions: [4, 3]}, new Float32Array([
          0.3093976, -1.2924036, -0.64339244, 1.1423386, 1.5052135, 1.8182521,
          -1.825652, -0.39694095, -0.90111053, 0.7807154, -1.9163561,
          -0.13988003
        ]));
    const output = nn.matmul(a, b);
    const model = await nn.createModel([{name: 'output', operand: output}]);
    ...
    const execution = await compilation.createExecution();
    execution.setInput(
        'a', new Float32Array([0.1309212, 0.9090703, 0.62183434, 0.9195683]));
    // output shape is [3]
  ...
  it('matmul 2d', async function() {
    ...
    const b = nn.constant(
        {type: 'tensor-float32', dimensions: [4, 3]}, new Float32Array([
          0.17467105, -1.2045133, -0.02621938, 0.6096196, 1.4499376, 1.3465316,
          0.03289436, 1.0754977, -0.61485314, 0.94857556, -0.36462623, 1.402278
        ]));
    const output = nn.matmul(a, b);
    const model = await nn.createModel([{name: 'output', operand: output}]);
    const compilation = await model.createCompilation();
    ...
    execution.setInput(
        'a', new Float32Array([
          0.9602246, 0.97682184, -0.33201018, 0.8248904, 0.40872088, 0.18995902,
          0.69355214, -0.37210146, 0.18104352, 3.270753, -0.803097, -0.7268995
        ]));
    // output shape is [3, 3]
    ...
    await execution.startCompute();
    const expected = [
      1.5347629, -0.3981255, 2.6510081, -0.14295794, 0.6647107, -0.70315295,
  ...
  it('matmul 3d', async function() {
    ...
    const b = nn.constant(
        {type: 'tensor-float32', dimensions: [2, 4, 3]}, new Float32Array([
  ...
  it('matmul 4d', async function() {
    ...
    const b = nn.constant(
        {type: 'tensor-float32', dimensions: [1, 2, 4, 3]}, new Float32Array([
