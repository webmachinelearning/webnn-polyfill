...
describe('test matmul', function() {
  ...
  it('matmul 1d', async function() {
    ...
    const a = builder.input('a', {type: 'float32', dimensions: [4]});
    ...
    const inputs = {
      'a': {
        buffer:
            new Float32Array([0.9025404, 0.89538723, 0.16789329, 0.7440875]),
      },
    };
  ...
  it('matmul 1dx2d', async function() {
    ...
    const a = builder.input('a', {type: 'float32', dimensions: [4]});
    const b = builder.constant(
        {type: 'float32', dimensions: [4, 3]}, new Float32Array([
          0.3093976,
          -1.2924036,
          -0.64339244,
          1.1423386,
    ...
    const inputs = {
      'a': {
        buffer: new Float32Array([0.1309212, 0.9090703, 0.62183434, 0.9195683]),
      },
    };
    const outputs = await compiledModel.compute(inputs);
    // c's shape is [3]
  ...
  it('matmul 2d', async function() {
    ...
    const outputs = await compiledModel.compute(inputs);
    // c's shape is [3, 3]
    utils.checkShape(outputs.c.dimensions, [3, 3]);
    const expected = [
      1.5347629,
      -0.3981255,
      2.6510081,
  ...
  it('matmul 3d', async function() {
    ...
    const b = builder.constant(
        {type: 'float32', dimensions: [2, 4, 3]}, new Float32Array([
    ...
    const outputs = await compiledModel.compute(inputs);
    // c's shape is [2, 3, 3]
    utils.checkShape(outputs.c.dimensions, [2, 3, 3]);
    const expected = [
      -0.10833447,
      -0.13393278,
      0.8061598,
  ...
  it('matmul 4d', async function() {
    const builder = nn.createModelBuilder();
    const a = builder.input('a', {type: 'float32', dimensions: [1, 2, 3, 4]});
    const b = builder.constant(
        {type: 'float32', dimensions: [1, 2, 4, 3]}, new Float32Array([
    ...
    const outputs = await compiledModel.compute(inputs);
    // c's shape is [1, 2, 3, 3]
    utils.checkShape(outputs.c.dimensions, [1, 2, 3, 3]);
    const expected = [
      1.2216457,
      -1.0545375,
      1.2706597,
