...
describe('test batchNormalization', function() {
  ...
  it('batchNormalization', async function() {
    ...
    const mean = builder.constant(desc, new Float32Array([0, 3]));
    const variance = builder.constant(desc, new Float32Array([1.0, 1.5]));
  ...
  it('batchNormalization with epsilon', async function() {
    ...
    const mean = builder.constant(
        desc, new Float32Array([0.3432895, 1.0855169, 1.8725895]));
    const variance = builder.constant(
        desc, new Float32Array([0.601868, 0.86580527, 0.38809904]));
    const scale = builder.constant(
        desc, new Float32Array([0.17215693, -0.7909758, 0.12456307]));
    const bias = builder.constant(
        desc, new Float32Array([0.5280557, -1.4475446, 0.1760742]));
    const epsilon = 1e-2;
