describe('test pool2d', function() {
  ...
  it('maxPool2d', async function() {
    const input =
        nn.input('input', {type: 'tensor-float32', dimensions: [1, 1, 4, 4]});
  ...
  it('maxPool2d dilations', async function() {
    ...
    const dilations = [2, 2];
    const output =
        nn.maxPool2d(input, windowDimensions, undefined, undefined, dilations);
    const model = await nn.createModel([{name: 'output', operand: output}]);
    const compilation = await model.createCompilation();
    const execution = await compilation.createExecution();
    execution.setInput('input', new Float32Array([
  ...
  it('maxPool2d pads', async function() {
    const input =
        nn.input('input', {type: 'tensor-float32', dimensions: [1, 1, 5, 5]});
  ...
  it('averagePool2d strides', async function() {
    ...
    const strides = [2, 2];
    const output =
        nn.averagePool2d(input, windowDimensions, undefined, strides);
    const model = await nn.createModel([{name: 'output', operand: output}]);
    const compilation = await model.createCompilation();
    const execution = await compilation.createExecution();
    execution.setInput('input', new Float32Array([
  ...
  it('global averagePool2d', async function() {
    const input =
        nn.input('input', {type: 'tensor-float32', dimensions: [1, 3, 5, 5]});
